...
- **`config/claude/skills/`**: home-manager によって `~/.claude/skills/` に配置される Claude 向けスキル (create-pr, ensure-branch, git-commit)。
- **`.agents/skills/`**: リポジトリローカルの共通スキル定義 (add-npm-package, add-script, issue-triage など)。

互換性のため、**`.claude/skills/`** は **`.agents/skills/`** を指すシンボリックリンクとします。

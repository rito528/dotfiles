...
- `&mut T` を含む型。可変参照の複製は aliasing ルールに反する
...
- 借用と所有の両方を自然に受けたい場合だけ、`Cow<'a, str>` や generic を検討する
...
| 必要なら借用、必要なら所有 | `Cow<'a, str>` | 書き換えや所有が必要なときだけ clone できる |
| 複数 thread で同じ文字列を共有 | `Arc<str>` | 所有値を共有し、文字列本体の重複を避ける |

`&str` を受けた後、すぐ `to_owned()` / `to_string()` して処理するなら、`&str` を選んだ意味が失われている可能性が高い。所有化してよいのは、値を struct に保存する、関数の外へ返す、別 thread / async task へ渡す、または API が所有値を要求する場合に限る。読むだけ、検証するだけ、分岐するだけ、別の借用 API へ渡すだけなら、借用のまま処理する。

```rust
// OK: 入力を読むだけなら &str のまま処理する。
fn is_valid_name(name: &str) -> bool {
    let name = name.trim();
    !name.is_empty() && name.chars().all(|c| c.is_alphanumeric() || c == '_')
}

// OK: 正規化した新しい文字列を返すので、戻り値として所有する。
...
// 避ける: 借用で読めるのに、先頭で所有化している。
fn is_valid_name_bad(name: &str) -> bool {
...
- `to_owned()` / `to_string()` が、保存、返却、所有値を要求する API 呼び出しのために必要か

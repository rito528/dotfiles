...
- ブランチ名（例: `fix/123-xxx`, `123-something` のように issue 番号を含む場合）
- `git log main..HEAD` のコミットメッセージ中の `#123` のような issue 参照
- これまでの会話でユーザーから伝えられた issue 番号
...
- Step 5 で対応する issue が特定できた場合、`## 概要` に `Closes #<issue番号>` を書く
- `Closes` キーワードは GitHub がマージ時に issue を自動で close する仕組みのため、必ずこの単語を使う
- 対応する issue がない場合は、この行自体を書かない

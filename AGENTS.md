...
5.  **AI によるコミットメッセージの安全な扱い**:
    - `git commit -m 'subject\n\nbody'` のように、`-m` 引数の中で `\n` を使って改行を表現してはいけません。
    - 本文や trailer を含むコミットメッセージでは、実際の改行を含む複数行メッセージを使ってください。
...
- **ディレクトリ構成詳細**:
    ...
    - **`modules/`**: 機能別に分割された Nix モジュール。
        ...
        - `codex.nix`: Codex CLI の `config.toml` と skill 配置
...
Claude Code で利用するスキル定義は用途ごとに 2 系統あります：
- **`config/agents/skills/`**: home-manager によって `~/.claude/skills/` と `~/.agents/skills/` にシンボリックリンクで反映される Claude 向けスキル (create-pr, ensure-branch, git-commit) の正規配置。
- **`.agents/skills/`**: リポジトリローカルの共通スキル定義 (add-npm-package, add-script, issue-triage など)。
- **`modules/codex.nix`**: `~/.codex/config.toml` と `~/.codex/skills/` を home-manager で宣言的に管理する。

リポジトリローカルでは、互換性のため **`.claude/skills/`** は **`.agents/skills/`** を指すシンボリックリンクとします。
...
## 5. Claude に対する許可 / 拒否コマンドの基準

`config/claude/settings.json` の `permissions` セクションで、Claude Code が実行できるコマンドを制御しています。
以下の基準に従って `allow` / `ask` / `deny` を分類しています。
...
- **外部からのデータ取得**: ネットワーク経由でデータをダウンロードする。
  - 例: `wget`, `curl`
- **Git の状態変更**: ステージング・コミットなど、リポジトリの履歴を変更する。
  - 例: `git add`, `git commit`, `git stash`, `git rm`
